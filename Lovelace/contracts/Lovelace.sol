 ...
 *
 * Flow (with on-chain result verification):
 *   1. Agent calls registerAgent() with MNT stake
 *   2. Client calls invokeAgent() with MNT escrow + optional resultSpecHash
 *      (keccak256 of accepted deliverable spec — commits what "done" means)
 *   3. Agent calls updateJob(jobId, resultUri, resultContentHash)
 *      where resultContentHash = keccak256 of the actual result content
 *   4. Agent calls closeJob()
 *   5. Client calls releasePayment() — contract checks resultContentHash != 0
 *      Optionally a trusted verifier can attest the hash via attestResult()
 *   6. Client calls rateAgent() after finalization
 */
contract Lovelace is ReentrancyGuard {
    ...
    struct Job {
        ...
        string description;
        string resultUri;
        bytes32 resultSpecHash;    // keccak256 of accepted deliverable spec (set by client at creation, 0 = not required)
        bytes32 resultContentHash; // keccak256 of actual result content (set by agent on updateJob)
        bool resultAttested;       // true if a trusted verifier confirmed the hash
    ...
    // Trusted result verifiers — any address the client explicitly adds per-job
    ...
    event ResultSpecSet(uint256 indexed jobId, bytes32 specHash);
    event ResultSubmitted(uint256 indexed jobId, address indexed agent, string resultUri, bytes32 contentHash);
    event ResultAttested(uint256 indexed jobId, address indexed verifier, bytes32 contentHash);
    event VerifierAdded(uint256 indexed jobId, address indexed verifier);
    ...
    function invokeAgent(
        ...
        address arbiter,
        uint16 arbiterFeeBps,
        bytes32 resultSpecHash  // keccak256 of the accepted deliverable spec; pass bytes32(0) to skip verification
        ...
        jobs[jobId] = Job({
            ...
            description: description,
            resultUri: "",
            resultSpecHash: resultSpecHash,
            resultContentHash: bytes32(0),
            resultAttested: false,
            parentJobId: 0,
            activeChildren: 0,
        ...
        if (resultSpecHash != bytes32(0)) emit ResultSpecSet(jobId, resultSpecHash);
    ...
    function updateJob(uint256 jobId, string calldata resultUri, bytes32 resultContentHash)
        external jobExists(jobId)
    {
        ...
        require(resultContentHash != bytes32(0), "Content hash required");

        job.status = JobStatus.InProgress;
        job.resultUri = resultUri;
        job.resultContentHash = resultContentHash;

        emit JobUpdated(jobId, resultUri);
        emit ResultSubmitted(jobId, msg.sender, resultUri, resultContentHash);
    }

    // ─────────────────────────────────────────────────────────────
    // 6b. Add Verifier (client nominates a trusted address per job)
    // ─────────────────────────────────────────────────────────────

    function addVerifier(uint256 jobId, address verifier) external jobExists(jobId) {
        require(jobs[jobId].client == msg.sender, "Not the client");
        require(verifier != address(0), "Zero address");
        jobVerifiers[jobId][verifier] = true;
        emit VerifierAdded(jobId, verifier);
    }

    // ─────────────────────────────────────────────────────────────
    // 6c. Attest Result (trusted verifier confirms content hash)
    // ─────────────────────────────────────────────────────────────

    function attestResult(uint256 jobId) external jobExists(jobId) {
        Job storage job = jobs[jobId];
        require(
            jobVerifiers[jobId][msg.sender] || job.client == msg.sender,
            "Not authorised to attest"
        );
        require(job.resultContentHash != bytes32(0), "No result submitted yet");
        require(!job.resultAttested, "Already attested");
        job.resultAttested = true;
        emit ResultAttested(jobId, msg.sender, job.resultContentHash);
    }
    ...
    function releasePayment(uint256 jobId) external nonReentrant jobExists(jobId) {
        ...
        // On-chain verification: agent must have submitted a content hash
        require(job.resultContentHash != bytes32(0), "No result hash: agent must call updateJob first");
